...
const ClockWrapper = ({ stateObject, timerToggler }) => (
    <div className="ClockWrapper wrapper">
        <div className="work-rest-alert">
            <p style={ (!stateObject.is_running) ? styles.stopped : (stateObject.rest) ? styles.chillin : styles.werkin }>
                {
        ...
        <DigitalClockDisplay timer={stateObject.timer} is_running={stateObject.is_running} seperatorFlash={stateObject.seperator_flash} />
        <div className="timer-toggle container-fluid">
            <div className="row col-12 no-gutters timer-toggle-wrap">
                <Button className="button-timer-toggle btn btn-success btn-block" toggledOn={stateObject.is_running} labelOff="START" labelOn="STOP" size="large" background="green" backgroundHover="white" color="white" colorHover="black" handler={timerToggler} />
            </div>
        </div>
    </div>
)
