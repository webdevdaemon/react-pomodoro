...
class TaskListItem extends Component {
  ...
  render() {
    let taskItem = Object.assign({}, this.props, this.state)
    ...
    return (
      <li className='TaskListItem container-fluid'>
        ...
        <div className='task-list-item-top-row row no-gutters'>
          ...
          <div className='item-time col-2'>
            <div className='item-hours'>
              <p>
                {`${taskItem.taskItem.hours} ${(taskItem.taskItem.hours > 1) ? 'hrs' : 'hr'}`}
              </p>
            </div>
            <div className='item-minutes'>
              <p>
                {`${taskItem.taskItem.minutes} ${(taskItem.taskItem.minutes > 1) ? 'mins' : 'min' }`}
              </p>
            </div>
          </div>
          <div className='item-break col-1'>
            <p className=''>
              {`${taskItem.taskItem.rest_length} mins`}
            </p>
          </div>
        ...
        <div className='task-list-item-bottom-row row no-gutters'>
          <div className={'item-notes reveal-notes-' + (taskItem.taskItem.notes_open) ? 'open' : 'close'}>
            <p>
              {taskItem.taskItem.notes}
            </p>
          </div>
        </div>
      </li>
